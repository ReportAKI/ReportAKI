const RULE_ID = 'reportaki/no-unicode-escapes-in-jsx';
...
export default function format(results) {
	...
	lines.push(`Found ${findings.length} Unicode escapes; ${omitted} additional findings omitted. Replace all escapes in affected JSX. [reportaki-unicode-escapes] total=${findings.length} shown=${shown.length} omitted=${omitted}`);
